...
/**
 * Created by dengkaitao on 2018/1/1 21:11.
...
public class StarBar extends View {
    ...
    private int starSize;//星星高度大小，一般宽度等于高度
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        this.starDistance = mTypedArray.getDimensionPixelSize(
                R.styleable.RatingBar_starDistance, 0);
        this.starSize = mTypedArray.getDimensionPixelSize(
                R.styleable.RatingBar_starSize, 20);
        this.starCount = mTypedArray.getInteger(
                R.styleable.RatingBar_starCount, 5);
        this.starEmptyDrawable = mTypedArray.getDrawable(
                R.styleable.RatingBar_starEmpty);
        this.starFillBitmap =  drawableToBitmap(
                mTypedArray.getDrawable(R.styleable.RatingBar_starFill));
        mTypedArray.recycle();

        paint = new Paint();
        paint.setAntiAlias(true);
        paint.setShader(new BitmapShader(
                starFillBitmap,  BitmapShader.TileMode.CLAMP, BitmapShader.TileMode.CLAMP));
    }
